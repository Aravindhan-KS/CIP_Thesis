\chapter{Implementation and Results}

\section{Data Preparation and Experimental Setup}

This section presents the experimental setup used for implementing and evaluating the proposed system. It provides an overview of the dataset used in the study, including its source and key characteristics. The procedures for data loading and preprocessing are described to ensure proper preparation of the data for model training. Furthermore, the strategy used to divide the dataset into training and testing subsets is explained. 

\subsection{Implementation Environment}

The implementation was carried out using the Python programming language with several scientific computing and quantum computing libraries.

\begin{table}[h]
\centering
\caption{Software Environment Used for Implementation}
\begin{tabular}{|c|c|}
\hline
Tool / Library & Purpose \\
\hline
Python & Core programming language \\
NumPy & Numerical computation \\
Pandas & Data processing \\
Scikit-learn & Classical machine learning utilities \\
Matplotlib / Seaborn & Data visualization \\
Qiskit & Quantum circuit simulation \\
Qiskit Machine Learning & Quantum kernel and QSVM implementation \\
Google Colab & Development and execution environment \\
\hline
\end{tabular}
\end{table}

The implementation was executed using the Google Colab platform, which allows integration of Python libraries and cloud-based computational resources.

\subsection{Dataset Description}

The NASA Lithium-Ion Battery Dataset was used for this study. The dataset consists of battery cycle measurements collected during laboratory experiments.

Each battery file contains multiple operational cycles including:

\begin{itemize}
\item Charge cycles
\item Discharge cycles
\item Impedance measurements
\end{itemize}

Battery measurements include the following parameters:

\begin{itemize}
\item Voltage
\item Current
\item Temperature
\item Capacity
\item Time
\end{itemize}

\begin{table}[h]
\centering
\caption{Dataset Summary}
\begin{tabular}{|c|c|}
\hline
Parameter & Value \\
\hline
Total Battery Files & 35 \\
Valid Batteries Processed & 33 \\
Total Cycles Loaded & 7313 \\
Discharge Cycles Used & 4600 \\
Valid Cycles after Filtering & 2667 \\
\hline
\end{tabular}
\end{table}

\subsection{Data Loading}

Battery data stored in MATLAB (.mat) format was loaded using the \texttt{scipy.io} library. The MATLAB structures were parsed to extract discharge cycle measurements.

Only discharge cycles were used because they contain the relevant electrochemical behavior needed to analyze battery degradation patterns.

\subsection{Train-Test Split}

The dataset was divided into training and testing sets to evaluate the model performance.

\begin{table}[h]
\centering
\caption{Dataset Split}
\begin{tabular}{|c|c|}
\hline
Dataset Portion & Percentage \\
\hline
Training Set & 80\% \\
Testing Set & 20\% \\
\hline
\end{tabular}
\end{table}

\section{Test Cases}

Several test scenarios were created to validate the ability of the model to detect different battery degradation states.

\begin{table}[h]
\centering
\caption{Test Case Examples}
\begin{tabular}{|c|c|c|}
\hline
Test Case & Input Condition & Expected Risk Level \\
\hline
TC1 & Stable voltage and temperature & Very Low Risk \\
TC2 & Slight voltage fluctuation & Low Risk \\
TC3 & Moderate degradation trend & Medium Risk \\
TC4 & Rapid capacity decline & High Risk \\
TC5 & Severe degradation indicators & Critical Risk \\
\hline
\end{tabular}
\end{table}


\section{Results}

\subsection{Timestamp Alignment}

Battery cycles contain time arrays representing measurements recorded during each cycle. These timestamps were aligned with cycle indices to maintain consistency across multiple battery datasets.

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{Figures/Timestamp_Alignment.png}
\caption{Timestamp Alignment Across Battery Cycles}
\label{fig:timestamp_alignment}
\end{figure}

Timestamp alignment enables proper comparison of degradation patterns across batteries.

\subsection{Missing Value Handling}

Different cycle types contain different measurement fields. Therefore, missing values were handled using safe extraction techniques.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{Figures/missing_value_handling.png}
\caption{Handling Missing Values in Battery Dataset}
\label{fig:missing_values}
\end{figure}

Median-based imputation was applied to fill missing entries while preserving statistical characteristics of the dataset.

\subsection{Statistical Feature Engineering}

Time-series battery signals were transformed into statistical descriptors to provide structured inputs for machine learning models.

The following statistical features were extracted:

\begin{itemize}
\item Mean
\item Standard deviation
\item Minimum value
\item Maximum value
\item Median
\end{itemize}

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{Figures/statistical_feature_extraction.png}
\caption{Statistical Features Extracted from Battery Cycles}
\label{fig:statistical_features}
\end{figure}

Electrochemical impedance parameters such as resistance components were also extracted to capture degradation behavior.

\subsection{Dendrite-Oriented Feature Engineering}

Additional electrochemical indicators were extracted to detect early signs of dendrite formation.

One important feature used is the differential capacity curve defined as

\begin{equation}
\frac{dQ}{dV}
\end{equation}

Changes in the differential capacity curve can indicate structural degradation within the battery.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{Figures/5. Dendrite Specific Features.png}
\caption{Dendrite-Specific Features Extracted from Battery Cycles}
\label{fig:dendrite_features}
\end{figure}

These features capture subtle variations in battery behavior that may indicate early dendrite growth.

\subsection{Sliding Window Modelling}

Battery degradation is a gradual process. To capture temporal degradation patterns, sliding window segmentation was applied across battery cycles.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{Figures/sliding_window.png}
\caption{Sliding Window Segmentation for Temporal Degradation Analysis}
\label{fig:sliding_window}
\end{figure}

Within each window, degradation trends such as capacity slope and degradation variance were computed.

\subsection{Window-Level Feature Construction}

For each sliding window segment, several degradation trend features were extracted.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{Figures/8. Feature Correlation Matrix.png}
\caption{Feature Extraction from Sliding Window Segments}
\label{fig:window_features}
\end{figure}

These window-level features represent temporal degradation patterns observed in battery cycles.

\subsection{Quantum Feature Mapping}

Principal Component Analysis (PCA) was used to reduce the dimensionality of the classical feature space before quantum encoding.

The original feature space was reduced to four principal components. These components were normalized to the interval $[0,\pi]$ for quantum circuit rotations.

Quantum feature encoding was then applied using parameterized quantum gates and entanglement operations.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{Figures/quantum_circuit.png}
\caption{Quantum Kernel Circuit Used for QSVM Feature Encoding}
\label{fig:quantum_circuit}
\end{figure}

The quantum circuit used for kernel generation in the QSVM model is shown in Figure~\ref{fig:quantum_circuit}.

\subsection{Quantum Kernel Support Vector Machine}

The classification stage uses a Quantum Kernel Support Vector Machine (QSVM).

The quantum feature map transforms classical feature vectors into quantum states as follows:

\begin{equation}
|\psi(x)\rangle = U_{\phi}(x)|0\rangle
\end{equation}

The similarity between two samples is computed using the quantum kernel function:

\begin{equation}
K(x_i,x_j)=|\langle\psi(x_i)|\psi(x_j)\rangle|^2
\end{equation}

The resulting quantum kernel matrix is used by a classical SVM classifier to separate battery risk categories.

The hybrid quantum-classical framework successfully identified degradation patterns within the battery dataset. The quantum feature transformation improved the separability of battery health states in the feature space.

\subsection{Risk Assessment}
The risk assessment results illustrate the distribution of batteries across different predicted risk categories obtained from the QSVM model. The risk distribution graph highlights the number of samples classified into each risk level, providing an overview of the model’s prediction outcomes. The prediction confidence plot further shows the confidence scores associated with each predicted class, indicating the reliability of the model’s classification results.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{Figures/risk.jpg}
\caption{Predicted Risk Distribution Analysis}
\label{fig:prediction_risk}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{Figures/prediction-confidence.jpg}
\caption{Prediction Confidence Distribution Analysis}
\label{fig:prediction_confidence}
\end{figure}

The predicted risk distribution and confidence levels obtained from the QSVM model are shown in Figure~\ref{fig:prediction_risk} and Figure~\ref{fig:prediction_confidence}, respectively.

\section{Performance Metrics}

To evaluate the effectiveness of the proposed system, several performance metrics were used.

\subsection{Accuracy}

Accuracy measures the proportion of correctly classified samples among all test instances.

\begin{equation}
Accuracy = \frac{TP + TN}{TP + TN + FP + FN}
\end{equation}

where $TP$, $TN$, $FP$, and $FN$ denote true positives, true negatives, false positives, and false negatives respectively.

\subsection{Recall}

Recall evaluates the model's ability to correctly identify actual anomalous battery conditions.

\begin{equation}
Recall = \frac{TP}{TP + FN}
\end{equation}

High recall is critical in EV battery monitoring because missing an anomaly may lead to severe safety risks.

\subsection{Feature Space Separability}

Feature Space Separability (FSS) measures the separation between normal and anomalous samples after quantum feature transformation.

\begin{equation}
FSS = \frac{\|\mu_{normal} - \mu_{anomaly}\|_2}{\sigma_{normal} + \sigma_{anomaly}}
\end{equation}

Higher values of FSS indicate better class separability.

\subsection{Variance Amplification Ratio}

Variance Amplification Ratio (VAR) quantifies the increase in feature variance produced by the quantum transformation.

\begin{equation}
VAR = \frac{Var(Q(x))}{Var(x)}
\end{equation}

where $x$ represents the classical feature vector and $Q(x)$ represents the quantum-transformed feature vector.

%to do performance metrics table

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{Figures/confusion-matrix.jpg}
\caption{Confusion Matrix of the QSVM Model}
\label{fig:confusion_matrix}
\end{figure}

Figure~\ref{fig:confusion_matrix} presents the confusion matrix of the QSVM classifier.

\subsection{Performance Evaluation of QSVM}

\begin{itemize}
\item Accuracy: 63.84\% with 6 kernels
%\item Accuracy: 87.32\% with 4 kernels
\item Recall: 0.7509
\item FSS: 0.4929
\item VAR: 0.4032 
\end{itemize}

\section{Summary of Results}
The results demonstrate that the hybrid quantum-classical learning framework improves the representation of battery degradation patterns and enables early detection of lithium dendrite risk.